...
function StaffNode({ emp, onDrop, onSelect, pos, canDrag, canDrop, isSelected }) {
  ...
  const [{ over }, drop] = useDrop(() => ({
    accept: 'staff',
    drop: (item) => canDrop && onDrop(item.id, emp.id),
    canDrop: () => canDrop,
    collect: (m) => ({ over: m.isOver() && m.canDrop() }),
  }));

  const img = `https://www.gravatar.com/avatar/${md5((emp.email || '').toLowerCase())}?s=40&d=identicon`;

  return (
    <Box
      ref={(n) => { if (canDrag) drag(n); if (canDrop) drop(n); }}
      ...
      sx={{
        ...
        border: over ? '2px dashed #D4AF37' : (isSelected ? '2px solid #D4AF37' : '1px solid #E2E8F0'),
        borderRadius: 2,
        p: 1.5,
        bgcolor: 'white',
        minWidth: 200,
    ...
    >
      <Box display="flex" alignItems="center">
        <Avatar src={img} sx={{ width: 36, height: 36, mr: 1.5, border: '1px solid #D4AF37' }} />
...
function HierarchyView({ refreshTrigger, onNotification }) {
  ...
  const loadTree = async () => {
    try {
      ...
      // check data and build the tree
      if (data && Array.isArray(data) && data.length > 0) {
        // use the first person (CEO) as root
